...
df_series_top100 <- read.csv("series_dataframe_top100.csv")

df_series_top500 <- read.csv("series_dataframe_top500.csv")

df_series_top1000 <- read.csv("series_dataframe_top1000.csv")
  ...
  df_series_sub3 <- reactive({
    ...
    df %>% 
      ...
      mutate(count = n(),
             ...
             otherVar = round(mean(!!sym(othervar)), 1)) %>% 
      ungroup %>% 
        ...
        if(req(input$plots_menu) == "overall"){
          ...
          if(req(input$plots_menu) == "additional"){
            ...
            df <- df_series_top1000 %>% 
              ...
              mutate(count = n(),
                     mostPopSeries = first(seriesTitle),
                     otherVar = round(mean(!!sym(other_var)), 1)) %>% 
              ungroup
